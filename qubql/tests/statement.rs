use qubql::{error::ResultExt, statement::{into_statements, stm::Statement, typ::Type}, token::into_tokens, util::Str};
...
//                         };
//                        "#;
//     let tokens = into_tokens(text).finish_parsing().unwrap();
//     let statements = into_statements(tokens).finish_parsing().into_fancy().unwrap();
// 
//     assert!(statements.len() == 1);
//     match &statements.get(0).unwrap().kind {
//         StatementKind::Create(x) => match x {
//             CreateStatement::CreateTableStatement(x) => {
//                 assert_eq!(x.table, Str::Borrowed("whatever"));
//                 assert_matches!(x.data, Type::Fields(_));
//                 match &x.data {
//                     Type::Fields(x) => {
//                         assert_eq!(x.fields.len(), 4);
